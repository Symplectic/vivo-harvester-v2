...
By default setting the *xslTemplate* parameter to *example-scripts/example-elements/elements-to-vivo.xsl* will allow you to use the example default scripts shipped with the harvester.

**You may additionally wish to set the *zipFiles* parameter to false if you are intending to develop custom XSLT mappings, although be aware that this will significantly increase the storage requirements of the harvester's intermediate output**
